...
export const ScreenController: ScreenControllerType = {
  createScreen: asyncHandler(async (req, res) => {
    ...
    if (typeof theaterId !== "string" || !theaterId.trim()) return sendError(res, 400, "Invalid TheaterId");
  ...
  getScreenById: asyncHandler(async (req, res) => {
    const { id } = req.params;
    if (typeof id !== "string" || !id.trim()) return sendError(res, 400, "Invalid ScreenId");
