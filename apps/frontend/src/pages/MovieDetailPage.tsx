...
export default function MovieDetailPage() {
  ...
  return <div className="app-shell"><SiteHeader /><main className="page">
    <section className="detail-hero" style={heroStyle}><div><Link className="eyebrow" to="/movies">← Back to films</Link><h1 className="title">{movie.name}</h1><div className="facts"><span className="fact">{movie.durationMins ? `${movie.durationMins} MIN` : "FEATURE FILM"}</span>{movie.language && <span className="fact">{movie.language.toUpperCase()}</span>}{movie.director && <span className="fact">DIRECTED BY {movie.director.toUpperCase()}</span>}{movie.releaseDate && <span className="fact">{new Date(movie.releaseDate).getFullYear()}</span>}</div><p className="detail-copy">{movie.description || "A compelling story made for the big screen. Select a showtime below to plan your perfect cinema evening."}</p></div></section>
